...
class MigrateBatchExecutable extends BaseMigrateBatchExecutable {
  ...
  /**
   * @var mixed Election node id.
   */
  protected $electionNode;
  ...
  public function __construct(MigrationInterface $migration, MigrateMessageInterface $message, array $options = []) {
    ...
    $this->electionNode = $options['election_node'];
  }

  /**
   * Helper to generate the batch operations for importing migrations.
   *
   * (Overridden to change class on batch operations)
   *
    ...
    foreach ($migrations as $migration) {
      ...
      $operations[] = [
        ...
        [$migration->id(), $options + [
            ...
            'election_node' => $this->electionNode
          ]],
      ];
    }
   ...
   *
   * (Override file path in source configuration)
   *
  ...
  public static function batchProcessImport($migration_id, array $options, &$context) {
    if (empty($context['sandbox'])) {
      ...
      $context['sandbox']['operation'] = static::BATCH_IMPORT;
    }
    ...
    $definition['source']['election_node'] = $options['election_node'];
